...
- AI 模块的实施以 `apps/web/src/app/(ai)/ai/Decision Agent与RAG可测试实施路线图-2026-08-17.md` 为当前路线图；开始前先核对对应阶段的目标、工作项、测试和完成标志，不脱离路线图一次性铺开实现。
...
- AI 会话的交互模型和信息层级应以 Codex、ChatGPT 网页端的成熟体验为质量基准：用户消息、助手正文、工具调用、运行进度、引用、审批、错误和取消状态必须作为可区分的消息部件按时间顺序展示，不能把全部过程拼成一段普通文本。
- “与 Codex/ChatGPT 一致”指工具使用过程清晰、状态可感知、结果可核验、失败可恢复等交互原则一致，不要求复制其品牌、私有样式、内部推理或未公开实现；具体视觉必须继续使用 NextNest 现有设计语言、Tailwind 变量以及 `@workspace/ui` 的 shadcn/Radix 基础组件，禁止死色值。
- 每次工具调用必须使用面向用户的业务名称，并至少区分等待、运行、成功、失败、取消和等待审批状态。运行中显示明确进度，完成后默认收敛为简短结果摘要；需要查看时可展开安全的输入摘要、结果摘要、来源数量、耗时和稳定错误信息，但不得展示 Token、Cookie、内部服务地址、执行租约、系统提示词、模型私有推理或未经脱敏的原始敏感数据。
- 工具调用卡、助手正文和后续引用必须保持稳定关联；刷新、断线补拉或从历史重新进入 Thread 后，应恢复成与首次流式展示相同的顺序和终态，不能重复出现工具卡、丢失结果或退化成纯文本日志。
- 助手正文必须通过统一且适合流式输出的安全 Markdown 渲染器展示，至少支持标题、段落、有序/无序列表、强调、引用、链接、表格、行内代码和代码块；用户消息默认按纯文本展示，除非后续产品需求明确允许 Markdown。
- Markdown 渲染必须禁用或严格净化原始 HTML，拦截脚本、事件属性、`javascript:` 等危险协议；外部链接需要安全打开策略。流式阶段的未闭合 Markdown 不能导致渲染报错、明显跳动或历史消息与实时消息表现不一致。
- 代码块和宽表格必须有独立横向溢出处理，代码块应提供语言标识和复制入口；长链接、长单词、超长工具结果不得撑破消息区。桌面端和移动端都要验证键盘操作、焦点、ARIA 状态、对比度、减少动画偏好以及滚动行为。
- 引用不是普通 Markdown 文本编号。引用必须使用稳定的来源 ID 和权限校验结果渲染成可交互部件，支持定位原消息、提案、事件、投票汇总或正式决议；来源删除、版本变化或失权时显示明确状态，不能继续展示过期正文。
- AI 只回答当前用户有权访问的决策形成过程范围内的问题；一个 Thread 可以在后端重新鉴权和候选确认后讨论或比较多个项目、分区和决策，但不能依靠提示词、浏览器参数或模型自行扩大范围。面对 React 教程、天气、通用写作、无关代码生成等离题请求，应简短拒绝并引导回决策形成过程；不能因为已经调用了决策工具，就继续回答无关内容。离题拒答、候选消歧、跨项目权限过滤和工具范围不可由提示词绕过都必须进入固定评测集。
- AI UI 的每个最小实施步骤都要同时覆盖首次流、执行中、成功、空结果、失败、取消、重试和历史恢复中与本步有关的状态；不得先做只能演示成功路径的静态卡片，再把关键状态无期限留给后续阶段。
