...
- “复用既有业务闭环”不等于把旧版业务组件原样搬进新版页面。应优先复用旧版的 service、Hook、状态管理、权限判断、接口调用和领域逻辑，再按照新版 UI 的组件职责重新接入或改造展示层。
...
- 对于同一业务能力需要同时服务旧版和新版 UI 的情况，优先抽取无视觉语义的业务 Hook/逻辑供两套展示层复用；只有视觉和结构也一致时才直接复用完整业务组件。必要时可以为组件增加明确的视觉变体，但不得用临时覆盖样式堆叠出难以维护的兼容层。
