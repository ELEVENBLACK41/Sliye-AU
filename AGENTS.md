...
- 先理解目录结构和现有写法，再动代码。
- 最小化改动代码 ，不要随意填冗余代码 ，即使填了后续不用也要删掉
- 页面都默认用中文。
...
- AI 模块的总体实施以 `apps/web/src/app/(ai)/ai/doc/Decision Agent与RAG可测试实施路线图-2026-08-17.md` 为当前路线图。阶段 0、1、2 已经完成，其产品、权限、数据、UI、模型治理和评测资产继续保留；运行时收敛不会把这些阶段重新标记为未完成。
- 自 2026-09-01 起，在 C0～C11 收敛期间，以同目录 `AI运行时官方能力收敛与渐进回退执行计划-2026-09-01.md` 为唯一详细执行基线。`AI第二阶段实施计划-2026-08-25.md` 和 `AI第二阶段2.7-GPT式POST直出流改造计划-2026-08-26.md` 仅作为已完成阶段的历史实现、资产清单和临时回退依据，不再继续扩建旧 Runtime；`AI第三阶段实施计划-2026-08-28.md` 暂停执行。
- C11 收敛验收完成并同步更新路线图、第二阶段和第三阶段文档后，第三阶段才恢复执行；恢复时以官方 Agent Runtime 上逐项增加授权业务工具为新基线，不得自动回到自建 Tool Loop、文本增量事件、POST/GET 双流、lease、queue 或 reducer 方案。
