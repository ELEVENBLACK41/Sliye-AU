...
- AI 不是替人做决定，而是辅助提取提案、风险和会议纪要。

### 当前产品边界

当前版本只负责“一项决策如何产生”的过程，产品主链路在形成正式决议并完成过程回放后结束。
...
`DecisionTask` 等现有结构仅作为未来扩展预留，不代表当前需要实现对应接口和页面。“决策后的执行闭环”可以在未来作为独立阶段重新评估，未经产品边界确认不进入当前开发计划。
...
- 会中发起投票。
- 点击“结束会议”，状态变成 `ENDED`。
- 会议开始、结束、提案和投票全部写入 `DecisionEvent`。
...
10. 结束会议。
11. 打开回放页，按时间线查看整个过程。
12. 如果接入录制，点击“投票通过”直接跳到录像对应时间。
13. 如果接入 AI，查看 AI 会议摘要和风险点。
```
