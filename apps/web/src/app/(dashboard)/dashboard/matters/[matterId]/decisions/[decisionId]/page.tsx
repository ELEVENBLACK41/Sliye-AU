/**
 * 本文件是议事内决策详情入口，只读取正式决策产物，不再创建独立决策群聊。
 */
...
/** 议事内决策详情动态参数。 */
type MatterDecisionRouteProps = {
  /** Next.js 16 异步路由参数。 */
  params: Promise<{ matterId: string; decisionId: string }>;
};

/** 渲染所属议事一致且当前成员可见的正式决策资料。 */
export default async function MatterDecisionRoutePage({ params }: MatterDecisionRouteProps) {
