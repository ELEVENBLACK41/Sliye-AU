...
> 文档状态：v0.13，2.7-A 已按 UI-first 修订、B～D 已完成，E～F 代码已完成待浏览器验收，G～H 待后续执行，I 的灰度/正式回滚部分后置
> 创建日期：2026-08-26  
...
2.7-F 代码已完成，待在真实新会话上完成首发、URL 切换、快速切换/刷新恢复和最终历史一致性的浏览器验收；2.7-E 的已有 Thread 浏览器验收也仍需补齐。按总路线图当前先做 2.8 基座盘点与清理；2.7-G 是否继续，待 2.8 盘点和老大确认后再决定，不自动进入后续步骤。
