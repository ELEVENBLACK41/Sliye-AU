...
> 状态：实施基线 v2.0（阶段 0、1、2、2.8、第三阶段 3.0、客户端状态收敛与后台 Run 活动提示已完成；当前进入 3.2-0 决策范围语义冻结与 AI 读取对齐）
> 日期：2026-08-17  
> 最近修订：2026-09-04
> 适用项目：NextNest（Next.js 16 + React 19 + NestJS 11 + Prisma 7 + PostgreSQL）  
> 关联笔记：[[AI]]、[[第一次讨论方案0817]]  
> 文档目的：把既有 AI 架构草稿拆成可逐步实现、逐步测试、可独立停下和可审查的工程路线。
> 当前执行基线：阶段 0、1、2、2.8、第三阶段 3.0、客户端状态收敛与后台 Run 活动提示已完成；当前执行 3.2-0 决策范围语义冻结与 AI 读取对齐，详细边界以 `AI第三阶段实施计划-2026-08-28.md` 为准。原 Gate C 官方 Runtime 收敛方案已停止执行，仅保留 `AI运行时官方能力收敛与渐进回退执行计划-2026-09-01.md` 作为历史决策记录；不执行 C0～C11，也不以收敛或回退作为第三阶段准入条件。

### 2026-09-04 当前执行状态

- 阶段 0、1、2、第三阶段 3.0、客户端状态收敛与后台 Run 活动提示已完成；不撤销已经验收的产品、权限、数据、UI、模型治理和评测成果。
- 2.8 已完成；现有 Web 工具占位文件明确保留，不作为当前阶段阻塞项。当前进入 3.2-0 决策范围语义冻结与 AI 读取对齐。
...
> 当前状态：🟡 3.2-0 决策范围语义冻结与 AI 读取对齐待执行。第三阶段 3.0 与 AI 工作区客户端状态收敛已完成；`AI第三阶段实施计划-2026-08-28.md` 仍是第三阶段详细基线。第三阶段继续使用当前 `streamText` + `tools` + `stopWhen` Loop，不以 `ToolLoopAgent`、`useChat` 或 UI Message Stream 迁移作为前置条件。
...
2. `getDecisionContext`：决策标题、状态、所属项目、项目级/小组级范围、可选私有分区和参与范围等基本上下文；与实体发现组成 2.4 的最小工具链。
...
产品范围语义：`PROJECT` 决策的 `areaId = null`，面向项目成员可见；`AREA` 决策的 `areaId` 指向同项目私有小组群，面向小组成员可见。公共群可以通过消息、会议和决策关联表达，但不写入 `Decision.areaId`；历史 `areaId = null` 按正式项目级决策处理，不需要回填。
...
4. 当前执行 3.2-0 决策范围语义冻结与 AI 读取对齐，随后完成 3.2-A～C 和 3.2-D 工具覆盖与可观测推理基线，再按能力 profile 逐项推进决策形成和过程证据工具，完成“发现 → 上下文 → 过程证据 → 回答”的产品闭环。
