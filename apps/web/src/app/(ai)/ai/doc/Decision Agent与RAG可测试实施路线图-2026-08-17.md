...
> 第二阶段详细历史记录：`AI第二阶段实施计划-2026-08-25.md`。第三阶段业务能力历史基线：`AI第三阶段实施计划-2026-08-28.md`。当前运行时收敛期间的唯一执行基线：`AI运行时官方能力收敛与渐进回退执行计划-2026-09-01.md`。本文继续维护阶段顺序、上下游依赖和总体验收门禁；详细执行权按本段优先级解释。
...
| 异步索引      | `OutboxEvent` + 后续队列平台（BullMQ / Vercel Workflow / Vercel Queues） | PostgreSQL 保证业务事件可靠落盘，具体队列平台在第 7/10 阶段按部署与任务时长评估 |
| Redis 职责  | 后续索引、短期缓存和跨进程协调候选，不承担普通聊天 UI 流 | 只有真实索引、长任务或多实例需求证明收益后才引入                       |
| 状态与恢复     | `Thread / Message / Run / ToolCall / Source` 为当前主链；`AiEvent / AiStep` 作为迁移期历史兼容 | 普通聊天优先使用 AI SDK UI Message Stream/useChat；token 级续播与耐久运行按需评估 |
...
| 2. AI 状态持久化    | Thread/Message/Run 可审计基础；历史重载与已有事件兼容；现有 AI 工作台接入真实状态 | Prisma AI 模型与迁移；Thread 列表/详情/管理接口；现有 UI 的真实数据接入 | 跨进程续跑；用 Redis 代替权威状态；高级会话管理；重做 AI 页面 |
| 2R. 官方运行时收敛门禁 | 在不回滚第二阶段数据的前提下，将普通聊天主链收敛到 AI SDK Agent/UI Stream/useChat | 收敛计划 C0～C11；官方链路 Spike；消息格式兼容；主链切换与旧链停写审计 | 业务工具扩展；旧表立即删除；强制引入 Workflow/Redis |
...
这些阶段是最终验收门禁，不要求所有代码工作包完全串行。第二阶段独立文档保留已验收实现和历史门禁；当前普通聊天运行时以 2R 收敛计划为执行覆盖层，第三阶段业务能力在 C11 后重新进入。总路线图的阶段编号与收敛计划的 C 编号不是一一对应关系。若历史实施记录与当前收敛计划冲突，以“历史记录说明已经发生什么、收敛计划说明当前要执行什么、本文说明阶段依赖”为准，不得保留两套当前执行规则。阶段可以暂停，但不得跳过 0、2R、4 和 5 的验收门禁。
...
> 本阶段的唯一详细执行基线是同目录的 `AI第二阶段实施计划-2026-08-25.md`。本节只保留总体目标和上下游说明；具体字段、Agent 目标解析、数据库约束、来源权限策略、现有 UI 接入方式和每轮完成状态均以独立文档为准。
...
> 本阶段的唯一详细实施基线是同目录的 `AI第三阶段实施计划-2026-08-28.md`。本节只保留总体目标和上下游说明；具体工具合同、来源类型、迁移、权限、测试、每轮完成状态和完成门禁以独立文档为准。

> **当前状态：暂停扩展。** 第三阶段业务目标保留，但其 Runtime、Stream、descriptor 和普通聊天恢复相关内容在运行时收敛期间不作为当前执行基线。C11 通过后，以收敛后的 `ToolLoopAgent + AI SDK tool + UI Message Stream + useChat` 重新校正本阶段详细计划，再按 Workspace 能力 profile 逐项恢复。
...
包含第 0～4 阶段以及 2R 收敛门禁。结果是基于官方 Agent/UI Stream/useChat 的可测试、可持久化、只读、不会越权的 Workspace Agent，并具备可新建、切换、重命名、归档和恢复的第一版会话历史工作区。
...
包含第 10～11 阶段。结果是在官方基础历史/流恢复能力之上，按真实需求完成长上下文压缩、耐久长任务、端到端取消、Evidence Receipt 重试、观测、成本和灰度。
...
> 本节记录 2026-08 形成第二阶段时的原始开工顺序。当前不再作为开工指令；现阶段按 `AI运行时官方能力收敛与渐进回退执行计划-2026-09-01.md` 的 C0～C11 执行，C11 通过后再恢复第三阶段业务工具。

下一次开始编码时，建议只做下面这一小段，不要直接创建向量表：

1. 为第一场景写 10 条最小 Gold Query，其中至少 3 条权限/无答案测试。
2. 把 `/api/chat` 的硬编码模型迁到模型注册表，并加 AI SDK Mock 测试。
3. 按第二阶段独立实施文档的 2.1 将 Thread 收敛为无业务上下文对话容器，并冻结 AI 共享契约、稳定错误码和 Run 状态转换矩阵，不在同一轮继续 migration。
...
第一轮完成后应得到一个很小但真实的闭环：用户在 AI 工作台消息中明确指定一项决策，模型调用真实工具，Nest 完成目标解析后的权限校验，Run 历史终态可重载、同进程执行事件可补拉，测试能证明未越权；随后用户可以从历史列表重新进入或创建另一个独立 Thread。工具闭环是后续跨项目、投票、会议查询和 RAG 的地基，会话历史则把这个能力变成可持续使用的产品入口，两者按上述顺序完成，不一次堆叠开发。生产级断连、完整取消和 Evidence Receipt 重试仍在第 10 阶段完成。
