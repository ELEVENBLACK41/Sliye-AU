...
> 当前执行基线：阶段 0、1、2 已完成；阶段 2 与阶段 3 之间插入强制 Gate C，C0、C1、C2 已完成，下一步为 C3。C0～C11 期间以 `AI运行时官方能力收敛与渐进回退执行计划-2026-09-01.md` 为唯一详细执行基线，第二阶段及 2.7 文档仅作历史实现与临时回退依据，第三阶段暂停。C11 完成后再同步更新第二、第三阶段详细计划并恢复第三阶段。

### 2026-09-01 当前执行状态

- 阶段 0、1、2 已完成；收敛只替换其中部分自建运行时实现，不撤销已经验收的产品、权限、数据、UI、模型治理和评测成果。
- C0 基线冻结、C1 官方 Agent 契约与 C2 UI Message Stream Spike 已完成；当前唯一下一步是 Gate C 的 C3，不继续修补或扩建旧 Runtime，也不提前执行第三阶段 3.1-A 及后续工具工作。
- 本文中关于 `AiStep/AiEvent` 文本增量、lease/fencing、POST/GET 双流、浏览器自建 reducer、普通聊天 Redis 实时广播和 `runId + afterSequence` 恢复的描述，在 C0～C11 期间视为历史实现说明；最终取舍以收敛计划为准，并在 C11 统一清理本文语义。
- 后续产品阶段编号保持不变。Gate C 是阶段 2 与阶段 3 之间的一次运行时迁移门禁，不新增产品阶段，也不改变 Decision Agent、RAG、评测和会议能力的最终方向。
...
3. 通过 Gate C 把自建运行时渐进收敛到 AI SDK 7 官方 Agent、UI Message Stream、Transport 和 AI Elements 能力。
4. 在官方 Runtime 上恢复第三阶段，逐项增加受 NestJS 最终鉴权的真实业务工具。
...
| Redis 职责  | 普通短对话首轮收敛不依赖 Redis；后台任务、限流、缓存或 Token 级续流出现真实需求后再评估    | 避免用共享基础设施继续维持已经退出的文本事件协议，同时保留未来多实例扩展空间          |
| 状态与恢复     | 保留 `Thread / Message / Run` 摘要、`ToolCall / SourceDependency / Citation`；最终或部分 UIMessage 持久化 | 官方 Agent 与 UI Message Stream 承担执行和传输，PostgreSQL 保留业务历史与审计事实 |
| 第一阶段不引入   | LangChain、LangGraph、独立向量库、Kafka、Neo4j、多 Agent、微调       | 当前没有足够问题证明这些重型技术能带来净收益                            |

> 部署假设：NestJS 与承载 Web Agent Runtime 的 Next.js Node 服务先以常驻、单实例进程部署。Redis/BullMQ、Vercel Workflow 或 Vercel Queues 只在第 7/10 阶段根据索引、长任务、暂停恢复和部署形态评估；如果 Agent Runtime 改为短生命周期 Serverless，必须在对应耐久能力开放前选择可靠执行平台，不能继续承诺浏览器断开后由同一短生命周期请求完成。PostgreSQL 仍是业务事实和长期审计边界。
...
> 状态说明：本节是路线图创建时的历史快照。阶段 0、1、2 已于后续实施中完成；当前实现与待迁移资产以 Gate C 的 C0 基线记录为准，不能再把本节“当前缺口”直接当作新的待办清单。
...
1. **AI 可回答当前产品域内全部授权业务数据问题。** 范围包括项目、决策、提案、投票、会议、讨论和参与关系；当前仍止于决策形成与协作过程，不扩展到决议后的任务分配、执行跟踪和验收。
...
> Gate C 说明：产品数据流、NestJS 最终鉴权和 PostgreSQL 权威事实边界继续有效；自建 Runtime、AiStep/AiEvent 文本增量、lease、POST/GET 双流和浏览器事件归约部分由收敛计划逐步替换。C11 前以收敛计划的 owner map 为准，本文架构图不作为继续扩建旧 Runtime 的依据。
...
| Gate C. 官方 Runtime 收敛 | 🟡 C0 执行中 | 保留业务资产并以官方 Agent、UI Message Stream、Transport 和 AI Elements 替换自建通用运行时 | `ToolLoopAgent`、`createAgentUIStreamResponse`、`useChat`、`DefaultChatTransport`；消息级持久化与必要审计 | 新业务工具、Token 级续流、Workflow、Redis/Queues、数据库清理 |
| 3. 只读 Agent 工具 | ⏸️ 暂停 | 真实业务工具循环 | 在 Gate C 官方 Runtime 上逐项增加 AI SDK tool；Nest 只读接口 | 通用 SQL/HTTP 工具；第二套工具 Schema/Loop |
...
阶段 0、1、2 已经完成，不因 Gate C 替换部分运行时实现而撤销完成状态。当前禁止直接进入阶段 3：必须先按收敛计划依次完成 C0～C11，并在 C11 重新冻结第三阶段的官方 Runtime 基线。阶段 3 恢复后，阶段 4～12 的产品能力顺序保持不变；其中阶段 7 的队列选型、阶段 10 的持久长任务边界和阶段 11 的观测接入点，必须以收敛后的真实部署与运行数据重新确认，不能沿用旧 Runtime 的技术前提。

---

## 7. 第 0 阶段：锁定第一个纵向场景与评测合同

> 当前状态：✅ 已完成。既有 Gold Fixture、指标、权限/无答案样例继续作为 Gate C 和后续阶段的回归资产。
...
> 当前状态：✅ 已完成。模型注册表、Gateway 接入和 Mock/Smoke 基础继续保留；Gate C 只调整 Agent 调用入口与运行时所有权。
...
> 当前状态：🟡 C0 基线冻结与方案确认。唯一详细执行基线为 `AI运行时官方能力收敛与渐进回退执行计划-2026-09-01.md`。

### 目标

保留阶段 0～2 已验证的业务权限、模型治理、会话数据、来源审计、业务工具、UI 和评测资产，以旁路建立、逐项接入、主链切换、停止旧写入、观察后删除的方式，把自建模型执行调度、文本事件持久化、POST/GET 双流、浏览器事件归约、lease 和通用队列运行时收敛到当前项目版本对应的 Vercel 官方能力。

### 执行约束

1. C0～C11 必须按独立小步执行；C11 完成前不恢复第三阶段，不新增第三阶段业务工具。
2. 第一轮不引入 Workflow、Redis、Queues 或数据库 migration；断连、显式停止、跨实例取消和进程崩溃收口必须在删除旧 lease/runtime 前形成明确门禁。
...
> 当前状态：⏸️ 暂停。C11 完成前不得执行 3.0、3.1-A 或后续工具工作；`AI第三阶段实施计划-2026-08-28.md` 当前仅作待重定基线的历史计划。恢复前必须把运行时改为官方 Agent/Tool/UI Message Stream 所有权，并保证 NestJS 不维护第二套工具 Schema 或执行循环。

### 目标

让官方 Agent 使用真实业务工具完成多步取证，同时把工具范围、参数、权限和输出大小控制在 NextNest 手中。首个产品闭环仍然是“解释一项决策如何形成”，但最终能力可扩展到当前产品域内全部授权项目、决策、投票、会议和讨论问题。
...
本阶段不再把自研普通聊天事件协议、事件序号补拉、执行租约或动态 descriptor Schema 作为新增业务工具的前置条件；这些内容由 2R 收敛计划处理。
...
> Gate C 影响：`pgvector`、Embedding 和 Outbox 目标保留；Redis/BullMQ、Vercel Queues 或 Workflow 的选型到本阶段按部署、吞吐、重试和运维成本重新确认，不再让普通聊天的 AiEvent 实时流决定队列架构。
...
> Gate C 影响：普通短对话的停止、重试和断连边界前移到 C6；本阶段保留上下文压缩、Evidence Receipt 重试和经真实需求证明的持久长任务，不再以 `runId + afterSequence` 的 Token 事件重放作为完成门禁。

### 目标

让长决策、长对话和多步工具调用不会无限膨胀；在普通聊天已经具备基础历史恢复后，再按真实需求补齐长任务、token 级续播和耐久恢复。
...
> Gate C 影响：完整 OpenTelemetry 仍在本阶段引入，但关联对象改为 Agent、UI Message Stream、ToolCall、Run 摘要与 Gateway tags；不得默认记录敏感输入输出，Gateway 日志也不能替代权限、来源和业务审计。
...
- LangChain/LangGraph：AI SDK 官方 Agent 能力足以完成当前主链，先避免第二层抽象。
...
### 里程碑 A：官方 Agent 骨架与业务边界

包含第 0～4 阶段及插入其间的 Gate C。阶段 0、1、2 已完成；当前先完成 Gate C，再继续阶段 3、4。结果是基于官方 Runtime、可测试、可持久化、只读且不会越权的 Decision Agent，并具备可新建、切换、重命名、归档和恢复的第一版会话历史工作区。
...
包含第 10～11 阶段。结果是在 Gate C 已完成普通短对话停止、重试和断连边界的基础上，继续完成长上下文压缩、Evidence Receipt 重试、真正持久长任务的按需方案、观测、成本和灰度；不恢复普通聊天的文本事件重放体系。
...
1. 完成 Gate C 的 C0 基线冻结、状态引用、依赖与测试基线记录，停止扩建旧 Runtime。
2. 按收敛计划逐个完成 C1～C10；每轮只做一个可独立理解、验证和回退的最小步骤。
3. C11 完成完整回归、清理路线图语义并为第三阶段冻结新的官方 Runtime 基线。
4. 第三阶段只选择一个窄业务工具恢复实施，当前候选为 `getUserWorkspaceSummary` 或 `listDecisionProposals`，不得并行铺开。
5. 第三阶段及权限门禁稳定后，再按阶段 5～12 的原产品能力顺序推进。

当前 C0 不改 Agent 代码、Prisma、依赖、环境变量或旧 Runtime。收敛期间保留旧链路作为对照和可回退路径；只有新主链通过对应门禁后，才停止旧写入并按前向迁移顺序删除不再需要的实现。
...
NextNest 最值得做的不是“一个会聊天的 AI”，而是一个覆盖当前产品域授权数据、并能证明自己为什么这么回答的 Workspace Agent；决策形成过程仍是第一条产品主线：
