...
> 第二阶段详细实施基线：`AI第二阶段实施计划-2026-08-25.md`。第二阶段的字段、状态、事务、接口、权限变化和验收细节以该独立文档为准；本文只维护阶段顺序、上下游依赖和总体验收门禁。
...
| 2. AI 状态持久化    | Thread/Run/Event 可审计；历史重载与同进程事件补拉；现有 AI 工作台接入真实状态 | Prisma AI 模型与迁移；Thread 列表/详情/管理接口；现有 UI 的真实数据接入 | 跨进程续跑；用 Redis 代替权威状态；高级会话管理；重做 AI 页面 |
...
这些阶段是最终验收门禁，不要求所有代码工作包完全串行。第 2 阶段统一按独立文档 `AI第二阶段实施计划-2026-08-25.md` 定义的 6 个目标步骤推进，在状态地基完成后穿插一个第 3 阶段的真实单 Thread 工具闭环来验证模型，再继续会话历史和现有工作区的数据接入。若本文第二阶段的摘要与独立文档发生冲突，以独立文档为准，并同步修正本文；不得保留两套并行实施规则。这种交错不代表对应阶段已经验收完成：里程碑 A 结束前，第 0～4 阶段的全部完成标志仍必须满足；第一轮的 10 条 Gold Query 只是种子集，也必须扩展到第 0 阶段规定的数量后才能宣告该阶段完成。阶段可以暂停，但不得跳过 0、2、4 和 5 的验收门禁。
...
> 本阶段的唯一详细执行基线是同目录的 `AI第二阶段实施计划-2026-08-25.md`。本节只保留总体目标和上下游说明；具体字段、可选入口上下文、数据库约束、权限变化策略、现有 UI 接入方式和每轮完成状态均以独立文档为准。

### 目标

让一次 AI 对话不再只是 HTTP 流，而成为可以审计、请求取消、通过新 Run 重试、重新加载历史，并在同一服务进程仍存活时补拉断线事件和统计成本的领域运行。Thread 归属于创建用户，可以没有固定业务上下文，也可以保存项目、决策或会议页面带入的可选入口上下文；入口上下文只作为默认焦点，不是权限凭证或永久检索边界。
...
| --- | --- | --- | --- | --- |
| 2.1 契约、入口上下文与状态规则 | 冻结 Thread、可选入口上下文、Message、Run、Event 的共享语义和合法状态转换 | contracts 类型、稳定领域错误、状态转换矩阵及表驱动测试 | Prisma migration、HTTP 接口、UI | 类型检查通过，合法/非法状态与入口上下文组合测试确定性通过，字段和错误码完成评审 |
...
| 2.5 会话历史与权限接口 | 交付可供工作区消费的 Thread 管理和历史读取 API | Thread 列表/详情/消息分页；重命名、归档、恢复；来源依赖登记；入口根上下文与单 Run 来源的权限变化处理；对应 BFF 路由 | 会话搜索、文件夹、置顶、共享和物理删除 | 新建、分页、更新、归档、恢复和直接 URL 权限测试通过；失权后标题、消息、工具结果、引用和事件均不可旁路读取 |
...
- `AiThread`：归属于创建用户的对话容器；可以没有固定业务上下文，也可以保存至多一个项目、决策或会议入口上下文；入口上下文不是权限凭证；`activeRunId` 用于第一版单 Run 并发门禁。
...
- Thread 不强制绑定单项 Decision。用户可以从 AI 工作台创建无固定业务上下文的会话，也可以从项目、决策或会议页面带入一个可选入口上下文；入口上下文只作为默认焦点，所有工具仍由 NestJS 根据当前用户实时鉴权。Decision、Meeting 的所属项目和私有区域由 NestJS 解析，不信任浏览器补充派生范围。
...
6. 入口上下文标识：明显展示 Thread 是否从项目、决策或会议页面进入；没有入口上下文时显示为普通 AI 工作台会话。入口上下文不限制跨项目比较，Agent 每次调用工具仍需显式参数和实时权限校验。
...
- `POST /api/ai/threads`：接收可选的项目/决策/会议入口上下文、首条用户消息和幂等键，在同一事务中创建 Thread、消息与 Run；不传入口上下文表示从 AI 工作台创建普通会话。随后由当前 BFF Agent Runtime 原子领取并启动 Run，响应直接返回流，第一版不提供持久化空 Thread 的接口。
- `GET /api/ai/threads`：按游标、归档状态和可选入口上下文查询当前用户仍有权访问的 Thread。
...
- 创建 Thread 的幂等键至少按“当前用户 + 创建请求键”隔离，并把首条消息与可选入口上下文纳入冲突校验；后续消息的幂等键作用域为“当前用户 + `threadId`”，重试的幂等键作用域为“当前用户 + 旧 `runId`”。相同键但正文或目标不同必须返回稳定冲突错误，不得跨用户、跨 Thread 或跨旧 Run 去重。
...
- 每次读取列表、详情、消息、工具调用、引用和事件时都重新校验当前用户、可选入口根上下文以及相关 Run 的来源依赖。项目、决策或会议入口根上下文本身失权后，Thread 整体锁定，直接 URL 不得返回标题、消息或运行内容。
- Thread 保存工具实际读取以及回答实际引用的全部来源 ID。没有固定入口上下文的工作台会话可能包含多个项目的结果，因此单个来源删除或失权时不能无条件锁死整条 Thread；应锁定或隐藏受影响 Run 对应的助手消息、工具结果和引用，并显示不含原标题、摘要或可推断信息的中性占位。未受影响的历史和后续授权查询仍可继续使用。
- 工具执行前、工具结果持久化前以及每批模型事件落库/推送前都由 NestJS 重新校验当前目标和已读取来源。长连接期间当前 Run 的目标或来源权限被撤销时，NestJS 在一个事务中收敛当前 Run、清空 `activeRunId` 并使执行租约失效；只有入口根上下文失权时才同时锁定整个 Thread。BFF 收到拒绝后触发本地 Abort，所有迟到写入因租约失效被拒绝。
- 权限恢复后的消息或 Thread 解锁规则在第二阶段独立文档中评审。来源永久删除时审计记录仍由后端保留，但普通用户不能读取依赖该来源的生成内容；不能通过只恢复部分引用来源绕过仍不可见的工具读取来源。
...
- 同一用户可以创建多个无固定上下文或带不同入口上下文的独立 Thread，消息、Run、引用和入口上下文不会互相污染。
- 新建、切换、重命名、归档和恢复归档均通过真实接口完成；刷新页面能根据路由恢复当前 Thread。
- 用户无法从列表、直接 URL、消息接口或事件补拉读取其他用户及无权项目/私有区域的 Thread。
- 用户在会话创建后失去入口根上下文访问权时，重新打开历史不能看到旧标题、助手回答、工具结果或引用内容；只有单个 Run 来源失权时，仅受影响的消息、工具结果和引用被安全隐藏，其他授权历史不泄漏也不必整体报废。
...
- 用户不能读取其他用户的 Thread，也不能通过无入口上下文会话旁路读取无权项目或私有区域数据。
- 权限撤销或引用来源删除后重新读取旧 Thread，必须命中入口根上下文锁定或单 Run/消息隐藏策略，不能从标题、消息、工具结果、引用或事件旁路泄漏历史内容。
- 在流式回答中途撤销当前 Run 的目标或来源权限，撤销后的事件不能继续落库或推送，NestJS 必须原子收敛 Run、清空 `activeRunId` 并废弃执行租约；只有入口根上下文失权时才锁定整个 Thread。
...
上述工具集只是“解释一项决策如何形成”的第一批纵向工具，不是最终 Agent 的能力边界。第 2 阶段真实单工具闭环通过后，第 3 阶段还应按独立小步逐步补充跨业务结构化工具，例如当前用户项目列表、待参与投票、最近会议/通话和会议时长统计；每组工具都必须先定义窄输入、后端权限和评测样例，不能一次铺开全部模块。
...
2. 建中心工具注册表，统一保存：
   ...
   - 最大返回条数/字符数。
   - 超时、重试、是否允许并行。
3. Agent runtime context 只包含当前身份、可选入口上下文和请求级预算等运行态；不把某个入口项目当成永久权限范围。单个工具通过 scoped context 只拿到自己需要的字段，并由 NestJS 根据实时权限解析允许范围。
...
2. 建统一 Retrieval API，输入至少包含当前用户身份、由 NestJS 解析的授权范围、可选项目/决策/会议/区域过滤、query、时间窗、source types 和 topK；无固定入口上下文的会话也必须先完成权限过滤，不能把“跨项目查询”解释为全库检索。
3. 第一版中文检索结合：
   - 精确标识符/标题命中。
...
3. 按第二阶段独立实施文档的 2.1 修正可选入口上下文、AI 共享契约、稳定错误码和 Run 状态转换矩阵，不在同一轮继续 migration。
...
8. 按 2.6 将真实 Thread、Run、工具、引用和页面四态接入已经完成的第一版 AI 工作台，不重新设计页面，并完成第 2 阶段整体验收。
9. 第 2 阶段验收通过后，再扩展 proposal/vote/resolution 工具。

第一轮完成后应得到一个很小但真实的闭环：用户在没有固定入口上下文的 AI 工作台会话中询问一项决策的基础信息，模型调用真实工具，Nest 完成权限校验，Run 历史终态可重载、同进程执行事件可补拉，测试能证明未越权；随后用户可以从历史列表重新进入或创建另一个独立 Thread。工具闭环是后续跨项目、投票、会议查询和 RAG 的地基，会话历史则把这个能力变成可持续使用的产品入口，两者按上述顺序完成，不一次堆叠开发。生产级断连、完整取消和 Evidence Receipt 重试仍在第 10 阶段完成。
