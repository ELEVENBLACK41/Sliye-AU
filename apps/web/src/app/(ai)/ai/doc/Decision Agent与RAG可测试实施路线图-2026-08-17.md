...
> 状态：实施基线 v1.4（阶段 0、1、2 已完成；当前执行 2.8 AI 地基完善与代码清理）
> 日期：2026-08-17  
> 最近修订：2026-09-02
> 适用项目：NextNest（Next.js 16 + React 19 + NestJS 11 + Prisma 7 + PostgreSQL）  
> 关联笔记：[[AI]]、[[第一次讨论方案0817]]  
> 文档目的：把既有 AI 架构草稿拆成可逐步实现、逐步测试、可独立停下和可审查的工程路线。
> 当前执行基线：阶段 0、1、2 已完成；当前先执行 2.8 AI 地基完善与代码清理，详细边界以 `AI地基完善与代码清理执行计划-2026-09-02.md` 为准。原 Gate C 官方 Runtime 收敛方案已停止执行，仅保留 `AI运行时官方能力收敛与渐进回退执行计划-2026-09-01.md` 作为历史决策记录；不执行 C0～C11，也不以收敛或回退作为第三阶段准入条件。

### 2026-09-02 当前执行状态

- 阶段 0、1、2 已完成；当前不撤销已经验收的产品、权限、数据、UI、模型治理和评测成果。
- 当前唯一下一步是 2.8：先盘点现有 AI 基座，补齐必要边界和验证，再清理无引用、过期、占位及经过证据确认的冗余代码。
...
| 2.8. AI 地基完善与代码清理 | 🟡 待开始 | 职责边界清楚、必要门禁保留、无关代码和占位入口清理 | 引用盘点、定向验证、单项删除/简化 | Runtime 迁移、数据库删改、Redis/Queues/Workflow、新业务工具 |
| 3. 只读 Agent 工具 | ⏸️ 待 2.8 完成 | 真实业务工具循环 | 基于当前 `streamText` + `tools` + `stopWhen` 逐项增加 AI SDK tool；Nest 只读接口 | 通用 SQL/HTTP 工具；第二套工具 Schema/Loop |
...
> 当前状态：🟡 待开始。详细执行基线为 `AI地基完善与代码清理执行计划-2026-09-02.md`。
...
> 当前状态：⏸️ 待 2.8 完成。`AI第三阶段实施计划-2026-08-28.md` 仍是第三阶段详细基线；第三阶段继续使用当前 `streamText` + `tools` + `stopWhen` Loop，不以 `ToolLoopAgent`、`useChat` 或 UI Message Stream 迁移作为前置条件。
...
包含第 0～4 阶段及 2.8。阶段 0、1、2 已完成；当前先完成 2.8，再继续阶段 3、4。结果是基于 AI SDK Core 与 NextNest 业务运行时、可测试、可持久化、只读且不会越权的 Decision Agent，并具备可新建、切换、重命名、归档和恢复的第一版会话历史工作区。
...
1. 按 `AI地基完善与代码清理执行计划-2026-09-02.md` 完成 2.8-0 基座盘点与职责冻结。
2. 逐轮完成 2.8-1 地基边界完善、2.8-2 无关代码清理和 2.8-3 防御性代码减法；每轮只做一个可独立理解、验证和回滚的最小步骤。
3. 通过 2.8-4 清理后门禁，确认当前 AI SDK Core 使用、权限、状态、持久化和恢复语义没有回归。
4. 进入第三阶段 3.0，再只选择一个窄业务工具推进，不并行铺开。
