...
2. `getDecisionContext`：决策标题、状态、所属项目/区域、参与范围等基本上下文；与实体发现组成 2.4 的最小工具链。
...
上述工具集只是“解释一项决策如何形成”的第一批纵向工具，不是最终 Agent 的能力边界。第 2 阶段会先验证 `findDecisionCandidates` + `getDecisionContext`，使普通用户无需手动输入 `decisionId`；第 3 阶段再按独立小步扩展提案、投票、决议、事件以及跨业务结构化工具，例如当前用户项目列表、待参与投票、最近会议/通话和会议时长统计。每组工具都必须先定义窄输入、后端权限和评测样例，不能一次铺开全部模块。
