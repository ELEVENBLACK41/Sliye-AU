...
2. `2.7.2 持久化模型与兼容迁移`：通过 Prisma 命令生成 migration，使 Thread 不再强制绑定单项 Decision，新增 Run 级范围与细粒度来源依赖所需的权威数据结构，并保证既有 2.6 Thread 可迁移、可读取和可回滚；本步不实现候选查询或 UI。
3. `2.7.3 授权决策发现与候选确认接口`：复用现有授权服务，在数据库侧先过滤权限，再按用户问题返回安全候选；同名或模糊候选等待用户确认，确认时重新鉴权；本步不接入模型工具循环。
4. `2.7.4 多决策 Run 运行闭环`：让同一 Thread 的不同 Run 使用各自已解析范围，支持一个 Run 读取和比较多个授权 Decision，并移除提示词不可切换 Thread Decision 的旧限制；本步仍只开放受控只读工具。
...
执行纪律：每轮只完成上述一个步骤。当前步骤通过类型检查、目标测试、兼容性检查和实施记录后必须停止，不自动继续下一步骤。
