...
> 状态：第 2 阶段 2.1～2.6 已完成；2.7 已确认实施并完成 2.7.1；2.8 UI 增量待实施
> 首版日期：2026-08-17
> 最近更新：2026-08-24
...
6. `2.7.6 工作区接入与整体验收`：移除创建会话前的强制 Decision 绑定，接入候选确认和跨项目/跨部门多决策对话，完成首次流、历史恢复、权限撤销、桌面和移动端端到端验收；如果 2.8 已先完成则直接复用活动组，否则只接入必要业务部件，不提前重做 2.8。
...
1. `2.8.1`：冻结活动组 ViewModel、业务文案映射、分组与收敛规则，并用 Fixture 覆盖全部运行状态；本步不改页面布局。
2. `2.8.2`：实现统一活动组组件和展开/收起交互，将现有普通工具卡迁移到紧凑行；本步不处理整个工作区视觉重构。
3. `2.8.3`：接入首次流、事件补拉和历史恢复，完成桌面/移动端、键盘、ARIA、减少动画和滚动验收；通过后删除仅由新组件完全替代且确认无引用的旧展示分支。
