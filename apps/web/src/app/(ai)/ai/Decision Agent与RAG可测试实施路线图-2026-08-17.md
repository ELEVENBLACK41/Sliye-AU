...
2. 再把模型接入、Agent 循环、持久化、工具权限做稳。
3. 先用结构化查询交付第一个可用闭环，证明产品价值。
4. 再建立关键词检索基线，之后才引入向量。
5. 向量上线后做混合召回、RRF、重排和逐条引用。
6. 最后再做长上下文压缩、生产级断连与取消加固、观测、灰度和会议多模态。
...
| 异步索引      | `OutboxEvent` + Redis + BullMQ Worker                  | PostgreSQL 保证业务事件可靠落盘，BullMQ 负责并发、重试、优先级和任务调度     |
| Redis 职责  | 任务队列、限流、短期缓存、实时事件广播                                    | 即使 Node 单实例也能服务后台任务；未来多实例时继续承担跨进程协调               |
| 状态与恢复     | 自建 `Thread / Run / Step / Event / ToolCall / Citation`；第 2 阶段先做历史重载与同进程事件补拉 | 借鉴 Codex 的运行思想，但状态模型必须服务 NextNest 的权限和审计需求        |
...
| Vercel AI SDK 7                | 流式 UI、ToolLoopAgent、结构化工具、Mock 模型、Embedding、Rerank、遥测、运行上下文 | 复用现有 Web 技术栈；模型调用层保持薄；测试不依赖真实模型                                            | AI SDK 不负责 NextNest 的业务权限、事实一致性和状态所有权                       |
...
| 阶段             | 可交付结果                   | 必须新增                                    | 暂不新增           |
| -------------- | ----------------------- | --------------------------------------- | -------------- |
| 0. 评测与边界       | Gold Set、指标、首个场景验收合同    | 测试夹具与评测脚本                               | 向量库、Agent 框架   |
| 1. 模型接入治理      | 可切换、可 Mock、可统计的模型层      | 模型注册表；可选开发期 DevTools                    | 多供应商业务分叉       |
| 2. AI 状态持久化    | Thread/Run/Event 可审计；历史重载与同进程事件补拉；第一版会话历史工作区 | Prisma AI 模型与迁移；Thread 列表/详情/管理接口与双栏 UI | 跨进程续跑；用 Redis 代替权威状态；高级会话管理 |
| 3. 只读 Agent 工具 | 真实业务工具循环                | AI SDK ToolLoopAgent；Nest 只读接口          | 通用 SQL/HTTP 工具 |
| 4. 权限与审批       | 越权召回为零、写操作有门禁           | AI Policy 与审批状态                         | OPA、完整沙箱       |
| 5. 结构化证据闭环     | 第一份带引用决策过程草稿            | 结构化证据 DTO 与引用协议                         | pgvector       |
| 6. 关键词基线       | 可测的文本检索 Recall@K        | PostgreSQL `pg_trgm`                    | Elasticsearch  |
| 7. 向量与异步索引     | 可重建的向量索引和可靠任务队列          | `pgvector`、Redis、BullMQ、Embedding       | 独立向量库、Kafka    |
| 8. 混合召回        | RRF 后的稳定候选集             | RRF 与候选调试视图                             | 复杂学习排序         |
| 9. 重排与证据合同     | 逐条引用、反方证据、降级            | Rerank 模型                               | 无依据自动写正式记录     |
| 10. 长任务与上下文    | 压缩；生产级断连加固；完整取消；Evidence Receipt 重试 | 压缩检查点、端到端 Abort、恢复压力测试                 | 多 Agent        |
| 11. 生产化        | 观测、成本、灰度、回滚             | OpenTelemetry/指标/功能开关                   | 全量立即开放         |
| 12. 高级能力       | 转写、多模态、证据图谱             | 仅在指标证明需要时引入                             | 默认 Neo4j、微调    |

这些阶段是最终验收门禁，不要求所有代码工作包完全串行。第 2 阶段统一按本阶段定义的 6 个目标步骤推进，在状态地基完成后穿插一个第 3 阶段的真实单 Thread 工具闭环来验证模型，再继续会话历史和工作区。这种交错不代表对应阶段已经验收完成：里程碑 A 结束前，第 0～4 阶段的全部完成标志仍必须满足；第一轮的 10 条 Gold Query 只是种子集，也必须扩展到第 0 阶段规定的数量后才能宣告该阶段完成。阶段可以暂停，但不得跳过 0、2、4 和 5 的验收门禁。
...
让一次 AI 对话不再只是 HTTP 流，而成为可以审计、请求取消、通过新 Run 重试、重新加载历史，并在同一服务进程仍存活时补拉断线事件和统计成本的领域运行。
...
| --- | --- | --- | --- | --- |
| 2.1 契约与状态规则 | 冻结 Thread、Message、Run、Event 的共享语义和合法状态转换 | contracts 类型、稳定领域错误、状态转换矩阵及表驱动测试 | Prisma migration、HTTP 接口、UI | 类型检查通过，合法/非法状态转换测试确定性通过，字段和错误码完成评审 |
...
| 2.4 真实单 Thread 运行闭环 | 用一个受权限保护的只读工具验证状态模型确实能承载 Agent Runtime | 穿插第 3 阶段的 `getDecisionContext`；BFF 执行器注册表；持久化事件；首次流、补拉、停止和重试链路；基础上下文预算 | 扩展 proposal/vote/resolution 工具、历史侧栏、高级压缩 | 一个真实请求可完成工具调用并保存真实 Run 终态；重连不重复启动执行器；越权请求被 NestJS 拒绝 |
| 2.5 会话历史与权限接口 | 交付可供工作区消费的 Thread 管理和历史读取 API | Thread 列表/详情/消息分页；重命名、归档、恢复；来源依赖登记；权限变化锁定；对应 BFF 路由 | 会话搜索、文件夹、置顶、共享和物理删除 | 新建、分页、更新、归档、恢复和直接 URL 权限测试通过；失权后标题、消息、工具结果、引用和事件均不可旁路读取 |
| 2.6 工作区与阶段验收 | 把持久化能力落实为可恢复使用的第一版产品入口 | 桌面双栏、移动端历史抽屉、路由恢复、Thread 切换、停止/重试、loading/empty/error/success、完整阶段测试 | 第 10 阶段的生产级长任务加固和 Evidence Receipt 重试 | 刷新和深链接可恢复 Thread；多个 Thread 不串线；桌面和移动端无双重滚动或截断；本阶段全部测试与完成标志通过 |
...
- `AiThread`：对话容器，绑定用户、项目，可选绑定决策/区域；`activeRunId` 用于第一版单 Run 并发门禁。
...
以下工作必须按 2.1～2.6 的目标步骤归属执行。2.4 穿插一个第 3 阶段的真实单 Thread 工具闭环来验证状态模型；该闭环通过后再进入 2.5 和 2.6。任一步可以暂停，但 6 个目标步骤全部完成前，第 2 阶段和里程碑 A 均不算完成。
...
7. 提供 Thread 创建、游标分页列表、详情和更新接口；列表按当前用户及其可访问的决策过滤，并默认按 `updatedAt` 倒序排列。
...
这部分是 Agent 状态持久化的产品入口，不是独立的通用聊天产品。它的目标是让用户能够找到、区分并继续之前针对某项决策发起的 AI 分析，同时保留来源、权限范围和运行审计。
...
- 历史项至少展示标题、所属项目、决策名称、最后更新时间和最近一次 Run 状态；没有真实字段时不使用静态数量或成员信息补位。
- 右侧继续保留消息、流式状态、工具调用、逐条引用、错误、停止和重试入口，接入历史时不得退化成只显示纯文本气泡。
- 第一版只允许创建绑定单项 Decision 的 Thread，不开放仅项目级、仅区域级或无业务上下文的通用会话；`projectId` 由 NestJS 根据 `decisionId` 解析并校验，不信任浏览器自行绑定。数据模型可以保留未来扩展所需的可空字段，但第一版 API 不接受这些额外范围。
...
6. 上下文标识：明显展示 Thread 当前绑定的项目和决策；如果需要改变 Decision，应创建新 Thread，不在旧会话中静默切换权限上下文。
...
- `POST /api/ai/threads`：接收 `decisionId`、首条用户消息和幂等键，在同一事务中创建 Thread、消息与 Run；随后由当前 BFF Agent Runtime 原子领取并启动 Run，响应直接返回流，第一版不提供持久化空 Thread 的接口。
- `GET /api/ai/threads`：按游标、归档状态和可选 `decisionId` 查询当前用户仍有权访问的 Thread。
...
- 创建 Thread 的幂等键作用域为“当前用户 + `decisionId`”，后续消息的幂等键作用域为“当前用户 + `threadId`”，重试的幂等键作用域为“当前用户 + 旧 `runId`”；相同键只在服务端规定的有效期内重放同一响应，不得跨用户、跨 Thread 或跨旧 Run 去重。
...
- 每次读取列表、详情、消息、工具调用、引用和事件时都重新校验当前用户对 Thread 所属 Decision 的访问权；失去 Decision 访问权后，该 Thread 不再出现在普通历史列表，直接 URL 也不得返回标题、消息或运行内容。
- Thread 保存工具实际读取以及回答实际引用的全部来源 ID。若用户仍可访问 Decision，但任一来源已删除或当前不可见，第一版采用保守策略：锁定整个 Thread 内容并返回明确的 `AI_THREAD_SCOPE_CHANGED` 领域错误，不按消息片段猜测性脱敏，也不继续向该 Thread 发送新消息。历史列表只能隐藏该项，或返回不含原标题和业务摘要的中性“权限范围已变化”占位项。
- 工具执行前、工具结果持久化前以及每批模型事件落库/推送前都由 NestJS 重新校验 Decision 和全部已读取来源。长连接期间权限被撤销时，NestJS 在一个事务中锁定 Thread、把当前 Run 以 `scope_changed` 原因收敛为 `cancelled`、清空 `activeRunId`，最后使执行租约失效；BFF 收到拒绝后触发本地 Abort，所有迟到写入因租约失效被拒绝。撤销生效前已经合法送达的内容无法回收，但撤销后不能继续产生新的历史泄漏。
- 权限恢复且工具读取和回答引用记录的全部来源依赖重新可见后才可以解除锁定；只恢复引用来源、但仍有工具读取来源不可见时必须继续锁定。来源永久删除时 Thread 审计记录仍由后端保留，但普通用户不能读取其生成内容。后续如需细粒度消息脱敏，必须单独设计并评测，不能直接放宽第一版规则。
- `AI_THREAD_SCOPE_CHANGED`、`AI_THREAD_RUN_ACTIVE` 等稳定领域错误必须定义在 `packages/contracts`，BFF 只做映射，不能靠页面匹配错误文本。
...
- 同一用户可以在同一决策下创建两个独立 Thread，消息、Run、引用和上下文不会互相污染。
- 新建、切换、重命名、归档和恢复归档均通过真实接口完成；刷新页面能根据路由恢复当前 Thread。
- 用户无法从列表、直接 URL、消息接口或事件补拉读取其他用户及无权项目/私有区域的 Thread。
- 用户在会话创建后失去 Decision 或任一引用来源访问权时，重新打开历史不能看到旧标题、助手回答、工具结果或引用内容，并得到稳定的权限范围变化状态。
...
- 用户不能读取其他项目/私有区域绑定的 Thread。
- 权限撤销或引用来源删除后重新读取旧 Thread，必须命中锁定/拒绝策略，不能从标题、消息、工具结果、引用或事件旁路泄漏历史内容。
- 在流式回答中途撤销 Decision 或来源权限，撤销后的事件不能继续落库或推送，NestJS 必须原子锁定 Thread、把 Run 收敛为 `cancelled`、清空 `activeRunId` 并废弃执行租约。
- 恢复部分引用来源但仍保留一个不可见的工具读取来源时，Thread 继续锁定；只有全部来源依赖重新可见后才允许解锁。
...
关闭浏览器再打开，能够从会话历史找到并恢复之前的 Thread，看到 Run 最终状态和已持久化消息；失败或取消不会被误标成完成。第一版桌面双栏和移动端历史抽屉可用，但不要求高级搜索、文件夹或共享会话。
...
1. `getDecisionContext`：决策标题、状态、所属项目/区域、参与范围等基本上下文。
2. `listDecisionProposals`：提案列表和明确字段。
3. `getVoteRoundSummary`：仅返回允许公开的投票汇总，不返回匿名个人票。
4. `getDecisionResolution`：正式决议及版本。
5. `searchDecisionEvents`：按时间/事件类型查过程节点。
6. `searchDiscussionMessages`：按决策、区域、时间窗、关键词查可见消息。
...
2. 建中心工具注册表，统一保存：
   ...
   - 最大返回条数/字符数。
   - 超时、重试、是否允许并行。
3. Agent runtime context 只包含身份/项目等运行态；单个工具通过 scoped context 只拿到自己需要的字段。
...
Agent 能通过 Mock 和真实脱敏数据完成“读取决策—列提案—取投票汇总—给出简短回答”的完整只读循环，且每次工具调用可审计。
...
2. 建统一 Retrieval API，输入至少包含用户身份、Decision/Project/Area 范围、query、时间窗、source types、topK。
3. 第一版中文检索结合：
   - 精确标识符/标题命中。
...
- `content`：原文或规范化文本。
- `contextPrefix`：决策、区域、议题、时间段的简短上下文。
...
包含第 0～4 阶段。结果是可测试、可持久化、只读、不会越权的 Decision Agent，并具备可新建、切换、重命名、归档和恢复的第一版会话历史工作区。
...
包含第 10～11 阶段。结果是在第 2 阶段历史重载和同进程事件补拉之上，完成长上下文压缩、生产级断连加固、端到端取消、Evidence Receipt 重试、观测、成本和灰度。
...
下一次开始编码时，建议只做下面这一小段，不要直接创建向量表：

1. 为第一场景写 10 条最小 Gold Query，其中至少 3 条权限/无答案测试。
2. 把 `/api/chat` 的硬编码模型迁到模型注册表，并加 AI SDK Mock 测试。
3. 按 2.1 完成 AI 共享契约、稳定错误码和 Run 状态转换矩阵，不在同一轮继续 migration。
4. 按 2.2 完成 Prisma AI 模型、migration、事务创建、幂等、单 Run 门禁和事件序号地基。
5. 按 2.3 完成执行租约、写入 fencing、停止/重试语义和过期租约对账。
6. 按 2.4 穿插实现 `getDecisionContext` 一个真实只读工具，经 BFF 调用 Nest，并让现有 AI Chat UI 展示真实工具调用、流式回答和错误状态。
7. 上述真实闭环通过后，按 2.5 完成 Thread/Message 历史与管理接口、来源依赖和权限变化锁定策略。
8. 按 2.6 完成第一版桌面双栏、移动端历史抽屉和第 2 阶段整体验收。
9. 第 2 阶段验收通过后，再扩展 proposal/vote/resolution 工具。

第一轮完成后应得到一个很小但真实的闭环：用户问一个决策的基础信息，模型调用真实工具，Nest 完成权限校验，Run 历史终态可重载、同进程执行事件可补拉，测试能证明未越权；随后用户可以从历史列表重新进入或创建另一个独立 Thread。工具闭环是后续 RAG 的地基，会话历史则把这个能力变成可持续使用的产品入口，两者按上述顺序完成，不一次堆叠开发。生产级断连、完整取消和 Evidence Receipt 重试仍在第 10 阶段完成。
