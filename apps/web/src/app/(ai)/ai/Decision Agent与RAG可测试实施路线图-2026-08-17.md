...
> 状态：第 2 阶段 2.1～2.6 已完成；2.7.1～2.7.6、2.8.1～2.8.3 的代码与自动化验收已完成；2.7.6 与 2.8.3 的桌面/移动端登录态实机验收待补
> 首版日期：2026-08-17
> 最近更新：2026-08-24
...
| 2. AI 状态持久化    | Thread/Run/Event 可审计；历史重载与同进程事件补拉；第一版会话历史工作区；2.7 支持动态授权范围与多决策 Thread；2.8 交付紧凑型运行活动 UI | Prisma AI 模型与迁移；Thread 列表/详情/管理接口与双栏 UI；Run 级范围与细粒度来源依赖；统一活动组组件 | 跨进程续跑；用 Redis 代替权威状态；高级会话管理 |
...
这些阶段是最终验收门禁，不要求所有代码工作包完全串行。第 2 阶段的已完成基线按本阶段定义的 6 个目标步骤推进，在状态地基完成后穿插一个第 3 阶段的真实单 Thread 工具闭环来验证模型，再继续会话历史和工作区。2.7 是基线完成后新增并已确认实施的增量，不追溯改变 2.6 和第 2 阶段的完成结论，必须继续按 2.7.1～2.7.6 小步推进。2.8 是已经确认需要实施的 UI 增量，负责把 2.6 的工具卡和运行状态改成紧凑型活动流；它同样不回退 2.6 的完成状态，但完成后将成为后续全部工具与运行 UI 的强制规范。这种交错不代表其他对应阶段已经验收完成：里程碑 A 结束前，第 0～4 阶段的全部完成标志仍必须满足；第一轮的 10 条 Gold Query 只是种子集，也必须扩展到第 0 阶段规定的数量后才能宣告该阶段完成。阶段可以暂停，但不得跳过 0、2、4 和 5 的验收门禁。
...
2. 创建第一版 50 条中文 Gold Query，后续扩展到 100～200 条：
   ...
   - 范围类：React 教程、天气、通用写作、无关代码生成等与当前 Thread 决策形成过程无关的问题。
   - 精确核对类：标题存在前缀、后缀、空格或单字符差异时，不得把语义相近误报为完全一致。
3. 每条 Query 标注：允许的数据范围、相关来源 ID、禁止来源 ID、答案要点、是否应该拒答；范围类还要标注允许的简短引导语和禁止继续回答的内容。
4. 将评测拆成三层：
...
- 离题请求必须简短拒绝并引导回当前决策，不解释无关知识、不生成无关代码，也不能借已读取的决策上下文强行关联回答。
- 标题精确核对、提示词要求切换其他 Decision ID、通用闲聊三类用例必须形成固定回归测试。
...
第 2 阶段已完成基线固定拆成下面 6 个目标步骤。目标步骤是阶段级审查边界，不要求为了赶进度在一轮内塞入全部子工作；如果当前步骤仍然过大，应继续在当前步骤内完成一个可独立验证的最小增量，不得提前进入下一步骤。2.7 和 2.8 作为已确认的后续增量单独实施，不纳入这 6 个步骤的既有验收结论。
...
| 2.4 真实单 Thread 运行闭环 | 用一个受权限保护的只读工具验证状态模型确实能承载 Agent Runtime | 穿插第 3 阶段的 `getDecisionContext`；BFF 执行器注册表；持久化事件；首次流、补拉、停止和重试链路；基础上下文预算；当前决策范围拒答门禁 | 扩展 proposal/vote/resolution 工具、历史侧栏、高级压缩、完整 Markdown 工作区 | 一个真实请求可完成工具调用并保存真实 Run 终态；重连不重复启动执行器；越权请求被 NestJS 拒绝；离题请求不被回答，标题差异和 Thread 绑定范围被准确说明 |
...
| 2.6 工作区与阶段验收 | 把持久化能力落实为可恢复使用的第一版产品入口 | 桌面双栏、移动端历史抽屉、路由恢复、Thread 切换、停止/重试、统一流式 Markdown 消息渲染、工具状态卡、loading/empty/error/success、完整阶段测试 | 第 10 阶段的生产级长任务加固和 Evidence Receipt 重试 | 刷新和深链接可恢复 Thread；多个 Thread 不串线；实时与历史消息/工具状态一致；Markdown 安全且流式稳定；桌面和移动端无双重滚动或截断；本阶段全部测试与完成标志通过 |

> 进度记录：2.1～2.6 已于 2026-08-24 完成。2.7 和 2.8 是讨论后新增且已确认实施的增量；2.7.1 已完成，后续步骤必须等待逐轮确认。二者都不代表 2.6 未完成，也不作为第 2 阶段既有验收的补交项。
...
第一版工作区的交互模型和信息层级以 Codex、ChatGPT 网页端的成熟工具使用体验为质量基准，但不复制其品牌、私有样式、内部推理或未公开实现。NextNest 继续使用现有设计语言、Tailwind 变量和 `@workspace/ui` 的 shadcn/Radix 组件，并遵守禁止死色值的项目规范。

- Thread 时间流必须把用户消息、助手正文、工具调用、运行进度、引用、审批、错误和取消展示为不同的结构化消息部件，不能把工具执行过程拼进助手正文或退化成调试日志。
- 工具卡使用面向用户的业务名称，2.6 至少支持等待、运行、成功、失败和取消状态。运行中显示简洁状态，完成后默认收敛为结果摘要；展开详情只展示经过脱敏的输入/结果摘要、来源数量、耗时和稳定错误，不展示租约、凭据、内部地址、系统提示词或模型私有推理。`waiting_approval` 展示协议在本阶段保持可兼容，并在第 4 阶段启用审批门禁时完成对应交互验收，不能为了提前展示而开放写工具。
- 助手正文使用统一、安全、适合流式增量的 Markdown 渲染器；至少支持标题、段落、有序/无序列表、强调、引用、链接、表格、行内代码和代码块。用户消息第一版保持纯文本。
- Markdown 禁用或严格净化原始 HTML，拦截脚本、事件属性和危险 URL 协议；流式阶段未闭合的 Markdown 不得导致报错或明显布局跳动，历史重载必须复用同一个渲染组件和安全策略。
- 代码块提供语言标识、复制入口和横向滚动；宽表格、长链接、长单词和超长工具摘要不能撑破消息区。外部链接使用安全打开策略。
- 工具调用、最终主张和引用保持稳定关联。引用使用后端允许的来源 ID 渲染为独立可交互部件，不把模型生成的普通 Markdown 编号当作可信引用。
- 工具状态与消息顺序必须从持久化 Run/Event/ToolCall 恢复；断线补拉、刷新和历史重进不得重复工具卡、丢失终态或改变首次流中的顺序。
- 桌面与移动端都要验证自动滚动不抢夺用户阅读位置、键盘操作、焦点可见、ARIA 状态、对比度、减少动画偏好，以及代码块/表格的溢出行为。
...
- Markdown 标题、列表、表格、引用、链接、行内代码和代码块在流式中间态、完成态与历史恢复态表现一致；危险 HTML/URL 测试被拦截，未闭合语法不会使消息区报错。
- 工具卡在等待、运行、成功、失败和取消状态下均有可访问的明确反馈；刷新或补拉后不重复、不乱序，完成态可展开安全摘要且不会泄露内部执行信息。等待审批状态在第 4 阶段启用时按同一消息部件协议补充验收。
...
- 流式 Markdown 使用分片输入覆盖未闭合强调、列表、表格和代码围栏；完成后与一次性渲染结果语义一致，并通过 XSS、危险协议和超长内容测试。
- 对每一种工具状态执行首次流与历史恢复对照测试，验证工具调用 ID、消息顺序、结果摘要、错误和引用关联一致。
...
### 2.7 增量：动态授权范围与多决策 Thread

#### 增量定位

2.6 已经完成的第一版以单 Decision 绑定保证范围稳定和权限安全。后续产品目标不要求用户在创建会话前手动绑定组织、项目或决策，也不把一个 Thread 永久限制在某个 Decision。一个 Thread 可以连续查询、讨论和比较当前用户在不同组织中有权访问的多个项目与多个决策；系统仍需为每次 Run 保存实际解析出的数据范围和来源依赖，以保证权限、引用、历史恢复和审计可核验。

该增量已经确认实施，但不回退 2.6 的完成状态。实现时必须重新核对现有 Schema、Thread API、历史权限策略和 UI 状态，并继续按单一职责拆成可独立验证的小步骤。

#### 分步执行计划

1. `2.7.1 动态范围共享契约`：冻结 Run 范围解析状态、授权决策范围、权限过滤后的候选摘要和细粒度历史可见性语义；只修改 contracts、类型断言和文档，不改数据库与接口。状态：已完成。
2. `2.7.2 持久化模型与兼容迁移`：通过 Prisma 命令生成 migration，使 Thread 不再强制绑定单项 Decision，新增 Run 级范围与细粒度来源依赖所需的权威数据结构，并保证既有 2.6 Thread 可迁移、可读取和可回滚；本步不实现候选查询或 UI。状态：已完成。
3. `2.7.3 授权决策发现与候选确认接口`：复用现有授权服务，在数据库侧先过滤权限，再按用户问题返回安全候选；同名或模糊候选等待用户确认，确认时重新鉴权；本步不接入模型工具循环。状态：已完成。
4. `2.7.4 多决策 Run 运行闭环`：让同一 Thread 的不同 Run 使用各自已解析范围，支持一个 Run 读取和比较多个授权 Decision，并移除提示词不可切换 Thread Decision 的旧限制；本步仍只开放受控只读工具。状态：已完成。
5. `2.7.5 细粒度历史权限恢复`：把权限复核和来源依赖落实到 Run、助手消息、工具调用与引用，只隐藏受失权或删除来源影响的内容，保留用户消息和其他可见历史；本步不调整 2.8 视觉。状态：已完成。
6. `2.7.6 工作区接入与整体验收`：移除创建会话前的强制 Decision 绑定，接入候选确认和跨项目/跨部门多决策对话，完成首次流、历史恢复、权限撤销、桌面和移动端端到端验收；如果 2.8 已先完成则直接复用活动组，否则只接入必要业务部件，不提前重做 2.8。状态：代码与自动化验收已完成；桌面/移动端登录态实机验收待补。

执行纪律：默认每轮只完成上述一个步骤。老大明确合并边界时可以在同一轮连续完成指定步骤；本轮按明确要求合并完成 2.7.2～2.7.4，完成类型检查、目标测试、兼容性检查和实施记录后停止，不自动进入 2.7.5。

#### 已确认的产品决策

1. 查询范围以当前用户对具体资源的实际访问权限为准，不以当前组织作为唯一边界。用户参与过其他组织的项目且当前仍有访问权时，可以在同一 AI 会话中查询对应决策。
2. 用户不需要手动选择或绑定组织、项目、区域和 Decision；`AiThread` 是对话容器，不再承担永久业务范围边界。
3. 同一个 Thread 可以涉及多个组织、项目和 Decision，也可以进行跨项目、跨决策比较；每次 Run 只获取完成当前问题所需的最小授权证据集，不能把用户全部可见数据无上限注入模型。
4. 当用户表达存在多个授权范围内的候选 Decision 时，AI 必须展示经过后端权限过滤的候选项让用户确认，不得自行猜测，也不得通过候选名称泄露用户无权访问的组织、项目或决策是否存在。
5. 当前暂不使用页面来源作为默认决策提示或优先范围。是否在决策页面打开 AI 时传入可见但不具约束力的临时提示，留作后续独立产品决策；未确认前继续按“不绑定、不隐式继承页面范围”设计。
6. 用户后来失去某项来源的访问权限时，只隐藏依赖该来源的助手回答、工具结果和引用；用户自己的消息以及未受影响的其他回答继续显示，Thread 仍可继续使用。后续 Run 只能访问用户当时仍有权读取的数据。

#### 建议的工程方向

- `AiThread` 只保留用户归属、会话元数据和运行并发控制；现有 `projectId / decisionId` 如何迁移为可空兼容字段或独立范围记录，必须在实施时先做 Schema 与历史数据影响评审，不在路线图中提前假定唯一方案。
- 为每次 `AiRun` 持久化本次解析出的组织、项目、区域和 Decision 范围、确认状态及必要的范围版本；范围只能由 NestJS 根据当前用户权限解析和裁决，浏览器与模型都不能自行扩大。
- 将来源依赖细化到 Run、助手消息、工具调用和引用能够稳定关联的粒度。权限变化后按受影响内容执行服务端过滤或锁定，不再因为一个来源失权而默认锁死整个跨项目 Thread。
- 在真实决策工具之前增加受限的决策发现与消歧能力，例如 `searchAccessibleDecisions` 或等价领域接口。接口必须先执行权限过滤，再返回带组织和项目说明的安全候选摘要。
- 每次读取历史、执行工具、返回候选、补拉事件和打开引用时重新鉴权；历史中曾经合法读取过某项数据，不代表用户永久拥有该数据的读取权。
- 保持当前 AI 产品边界：动态范围只覆盖用户有权访问的决策形成过程，不把 AI 扩展为通用问答，也不允许跨用户共享私有上下文。

#### 本增量明确不做

- 不实现当前页面上下文提示或自动优先当前 Decision。
- 不允许模型以自然语言声明替代后端权限判断，也不建立绕过组织、项目或私有区域规则的“全局 AI 权限”。
- 不一次完成全部跨项目检索、向量索引或高级比较能力；每个发现、确认和工具读取能力仍按最小步骤实现。
- 不实现跨用户共享 Thread、协同编辑、物理删除或通用聊天。

#### 验收标准

- 用户无需预先选择组织、项目或 Decision，即可发起第一条与决策过程有关的问题。
- 同一 Thread 可以先查询组织 A 的 Decision，再查询或比较组织 B 的 Decision；前提是当前用户分别拥有对应资源访问权。
- 同名或表述模糊的 Decision 只展示当前用户有权访问的候选，并在用户确认前不读取候选正文或生成确定性结论。
- 无权组织、项目、私有区域和 Decision 不会出现在候选、工具结果、引用、错误文本或侧信道响应中。
- 一项来源失权后，只有依赖该来源的助手回答、工具结果和引用不可见；未受影响内容仍能读取，Thread 可以继续使用，新的 Run 不会再次获取已失权来源。
- 既有 2.6 单 Decision Thread 在迁移后仍能读取和继续使用；数据迁移、接口兼容和回滚方式有明确验证记录。
- 新增跨组织、多决策、候选消歧、权限撤销和部分历史隐藏的固定 Gold Query 与端到端权限测试。

### 2.8 UI 增量：紧凑型工具与运行活动流

#### 增量目标与实施边界

2.8 对 2.6 已完成的消息、工具卡和运行状态 UI 修改一个正式版本：普通工具调用和运行步骤改用紧凑、按时间排列、可折叠的活动组；助手正文、候选确认、引用、审批和需要用户处理的错误继续保持独立消息部件。2.8 是已经确认的实施项，不再只是视觉参考；完成后，第 3～12 阶段新增的工具、检索、索引、评测、生成和其他运行过程都必须复用同一活动组规范与组件边界。

本增量只调整运行过程的信息架构、组件组合、交互状态和视觉表达，不重做整个 AI 工作区，不改变 2.6 已冻结的 Run/Event/ToolCall 权威状态语义，也不顺带实现 2.7。若现有持久化事件确实缺少稳定渲染所需的安全字段，应先提出最小契约增量并重新确认，不能由前端猜测或伪造步骤。

建议继续拆成三个可独立审查的最小步骤：

1. `2.8.1`：冻结活动组 ViewModel、业务文案映射、分组与收敛规则，并用 Fixture 覆盖全部运行状态；本步不改页面布局。状态：已完成。
2. `2.8.2`：实现统一活动组组件和展开/收起交互，将现有普通工具卡迁移到紧凑行；本步不处理整个工作区视觉重构。状态：已完成。
3. `2.8.3`：接入首次流、事件补拉和历史恢复，完成桌面/移动端、键盘、ARIA、减少动画和滚动验收；通过后删除仅由新组件完全替代且确认无引用的旧展示分支。状态：代码与自动化验收已完成；桌面/移动端登录态实机验收待补。

#### 视觉与交互方向

工具调用、模型运行、检索、评测、草稿生成和其他可观测执行过程，统一采用类似 Codex 运行活动区的紧凑型时间流表达。这里借鉴的是“当前动作摘要 + 可折叠步骤列表 + 完成后收敛”的信息层级，不复制 Codex 品牌、英文文案、命令行内容、颜色或底部状态栏。
...
1. 一个 Assistant Run 对应一个稳定的运行活动组，组头只使用一句面向用户的当前动作或完成摘要；步骤按照真实事件顺序追加，不能因为流式补拉或历史恢复而重排、重复或丢失。
2. 活动组展开时以轻量行展示步骤，不为每个普通工具默认渲染大面积 Card、边框和重复标题。每行只包含状态图标、业务动作名称和必要的安全摘要，长输入、长结果和技术详情默认隐藏。
3. 运行中保持展开并突出当前步骤；全部成功后默认收起为一行摘要。用户主动展开后可以查看各步骤的安全输入摘要、结果摘要、来源数量、耗时和稳定错误信息。
4. 等待、运行、成功、失败、取消和等待审批必须使用图标、文本与 ARIA 状态共同表达，不能只依靠颜色。颜色只能使用 NextNest 现有 Tailwind 变量和语义 Token，禁止写死色值。
5. 失败、等待审批、需要候选确认和需要用户重试的状态不能静默收起。它们应直接展示发生了什么以及下一步操作；候选 Decision、引用和审批仍是独立可交互消息部件，不塞进不可操作的普通步骤文本。
6. 文案必须使用用户理解的业务动作，例如“正在查找相关决策”“已读取投票汇总”“未能读取会议记录”，不得直接展示内部工具名、HTTP 路径、SQL、原始命令、模型私有推理、系统提示词、Token、Cookie、租约或内部服务地址。
7. Chevron 展开状态、运行图标和步骤更新可以使用克制的过渡动画；必须支持键盘操作、清晰焦点、`aria-expanded`、`aria-live` 和减少动画偏好，不能依赖持续闪烁制造运行感。
8. 首次流、断线补拉、刷新恢复和历史重进必须复用同一个活动组组件和事件映射。完成态摘要由持久化 Run、Event、ToolCall 和 Citation 结构生成，不能通过匹配助手正文推断。
9. 移动端保持相同的信息顺序，但允许隐藏次要元数据；活动组、宽结果和长文本不能撑破消息区，也不能引入第二层不可控纵向滚动。
10. 运行活动流只解释系统采取了哪些可审计动作及其结果，不展示或模拟模型思维链。没有真实结构化事件时，不得为了视觉效果伪造运行步骤。

#### 验收标准

- 单工具、多工具、无工具、部分失败、取消、等待审批和候选确认 Run 都能生成稳定且可理解的活动组。
- 运行中展开、成功后收起；失败或等待用户操作时保持可发现，用户可以通过鼠标和键盘展开或收起。
- 普通运行步骤不再形成连续的大卡片墙；助手正文、候选确认、引用和需要操作的错误仍保持清晰的独立层级。
- 首次流与历史恢复的组头摘要、步骤数量、顺序、状态和来源数量一致，重复 Event 不会生成重复行。
- 所有展示文案均为业务名称和脱敏摘要，安全测试证明不会暴露内部工具参数、服务地址、执行租约、系统提示词或模型私有推理。
- 桌面端和移动端完成键盘、ARIA、对比度、减少动画、长内容溢出及滚动行为检查。
...
7. 工具调用 UI 使用与消息流分离的结构化部件，并遵守 2.8“紧凑型工具与运行活动流”规范，统一消费稳定工具状态和结果摘要；不得依赖匹配模型正文来推断工具是否执行、成功或失败。
...
- 同一工具调用在首次流、事件补拉和历史重载中的活动组、步骤状态、顺序及结果摘要保持一致；完成后能够稳定收起，失败和等待用户操作时保持可发现。
...
6. 按 2.4 穿插实现 `getDecisionContext` 一个真实只读工具，经 BFF 调用 Nest，并让现有 AI Chat UI 展示真实工具调用、流式回答和错误状态；同时加入当前决策范围拒答、标题精确比较和 Thread 绑定范围不可由提示词切换的回归测试。
7. 上述真实闭环通过后，按 2.5 完成 Thread/Message 历史与管理接口、来源依赖和权限变化锁定策略。
8. 按 2.6 完成第一版桌面双栏、移动端历史抽屉、统一安全流式 Markdown、结构化工具状态卡和第 2 阶段整体验收。
9. 2.7 已确认为“动态授权范围与多决策 Thread”增量，按 2.7.1～2.7.6 小步实施；应在大规模扩展业务工具和检索前完成，避免 Thread 范围、来源依赖和权限恢复策略发生更大返工。
10. 2.8 已确认为下一版 UI 增量，按 2.8.1～2.8.3 小步完成紧凑型运行活动流并替换 2.6 的普通工具卡展示。若 2.7 与 2.8 排入同一迭代，应先完成 2.7 的范围和来源依赖调整，再一次完成 2.8 UI 适配。
11. 2.8 验收通过后继续扩展 proposal/vote/resolution 工具；后续所有工具接口仍需保留未来由 Run 级授权范围调用的兼容边界，所有工具和运行过程必须复用 2.8 活动组规范。
...
2.7 增量将上述单 Decision 基线演进为无需用户手动绑定、可在授权范围内跨项目和多决策讨论的会话模型。它保留 2.6 的状态持久化能力，但把业务范围从 Thread 永久绑定下沉为每次 Run 的动态授权范围，并把权限变化处理从整条 Thread 锁定细化为只隐藏受影响回答。当前数据模型没有独立 Organization/Tenant 实体，因此 2.7 先按用户对项目、分区和决策资源的真实权限工作，不把部门伪装成租户边界；未来若引入多租户组织模型，需要单独确认领域与数据迁移方案。

2.8 UI 增量在上述状态地基上修改一版工具与运行展示：现有 2.6 普通工具卡收敛为紧凑型活动组，完成后默认收起，异常和待用户操作状态保持可发现。2.8 验收后，该组件和信息层级成为第 3～12 阶段的统一 UI 基线，后续不得重新为单个工具发明另一套运行卡片。
...
- [OpenAI ChatKit Thread Items API：消息、工具调用、任务与 Widget 分离建模](https://developers.openai.com/api/reference/typescript/resources/beta/subresources/chatkit/subresources/threads/methods/list_items)
