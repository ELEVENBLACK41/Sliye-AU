...
这些阶段是最终验收门禁，不要求所有代码工作包完全串行。第一轮可以先完成第 2 阶段的状态地基（2A），穿插一个第 3 阶段的真实单 Thread 工具闭环来验证模型；闭环通过后再完成第 2 阶段的会话工作区（2B）。这种交错不代表对应阶段已经验收完成：里程碑 A 结束前，第 0～4 阶段的全部完成标志仍必须满足；第一轮的 10 条 Gold Query 只是种子集，也必须扩展到第 0 阶段规定的数量后才能宣告该阶段完成。阶段可以暂停，但不得跳过 0、2、4 和 5 的验收门禁。
...
以下工作分成两个相邻工作包：2A 先完成状态模型、事务与最小恢复能力；2B 在一个真实单 Thread 工具闭环验证通过后完成会话历史工作区。2B 不阻塞验证第一个工具，但 2A 与 2B 都完成前，第 2 阶段和里程碑 A 均不算完成。
...
3. 完成 2A：设计并评审 `AiThread / AiRun / AiEvent` 最小状态模型，落地 migration、状态服务、幂等和事件序号地基。
4. 实现 `getDecisionContext` 一个真实只读工具，经 BFF 调用 Nest。
5. 让现有 AI Chat UI 展示这一次真实工具调用、流式回答和错误状态。
6. 上述真实闭环通过后完成 2B：实现本节定义的 Thread/Message/Run/Stream 最小接口、权限变化锁定策略和第一版双栏会话工作区；这一项不反过来阻塞单 Thread 工具闭环验证，但未完成前第 2 阶段和里程碑 A 不能验收。
7. 通过后，再扩展 proposal/vote/resolution 工具。
