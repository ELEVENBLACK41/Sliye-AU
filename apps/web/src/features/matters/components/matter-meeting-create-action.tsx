...
import type { DiscussionAreaSummary, MatterUserSummary } from '@workspace/contracts/matters';

import { DateTimePicker } from '@/components/date-time-picker';
import { createMatterMeeting } from '../services/matters-client.service';
...
/** 当前分区会议创建操作属性。 */
type MatterMeetingCreateActionProps = {
  matterId: number;
  area: DiscussionAreaSummary;
  decisions: DecisionSummary[];
  candidates: MatterUserSummary[];
};

/** 渲染会议信息、多决策和多参会人选择。 */
export function MatterMeetingCreateAction({ matterId, area, decisions, candidates }: MatterMeetingCreateActionProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    setError('');
    try {
      const meeting = await createMatterMeeting(matterId, {
        areaId: area.id,
        title: title.trim(),
      ...
      <SheetContent className="sm:max-w-xl">
        ...
        <form className="flex flex-1 flex-col gap-4 overflow-y-auto px-4" onSubmit={handleSubmit}>
          <div className="grid gap-2">
            <Label htmlFor="matter-meeting-title">会议标题</Label>
            <Input
              id="matter-meeting-title"
              value={title}
              required
              maxLength={120}
          ...
          <div className="grid gap-2">
            <Label htmlFor="matter-meeting-description">会议说明</Label>
            <Textarea
              id="matter-meeting-description"
              value={description}
              onChange={(event) => setDescription(event.currentTarget.value)}
            />
          </div>
          <DateTimePicker
            id="matter-meeting-time"
            label="计划时间"
            value={scheduledAt}
