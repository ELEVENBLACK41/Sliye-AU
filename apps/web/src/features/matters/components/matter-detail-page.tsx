/**
 * 本文件组合议事头部、可见分区导航、聊天主区和决策、会议、成员信息。
 */
...
import { MatterAdminActions } from './matter-admin-actions';
import { MatterDecisionCreateAction } from './matter-decision-create-action';
import { MatterMeetingCreateAction } from './matter-meeting-create-action';
import { MatterChatPanel } from '../chat/components/matter-chat-panel';
...
type MatterDetailPageProps = {
  ...
  canManageMatter: boolean;
  canCreateMeeting: boolean;
  decisionFilterId?: number;
};

/** 议事状态的中文文案。 */
const statusText: Record<MatterDetail['status'], string> = {
  ACTIVE: '进行中',
  CLOSED: '已关闭',
  ARCHIVED: '已归档',
};
    ...
    <main className="space-y-4">
      ...
      <Card className="rounded-md shadow-none">
        <CardHeader className="gap-3">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-sm text-muted-foreground">
                议事 #{matter.id} · {matter.department.name}
              </p>
              <CardTitle className="mt-1 text-2xl">{matter.title}</CardTitle>
            </div>
          ...
          <p className="text-sm leading-6 text-muted-foreground">{matter.description || '暂无议事说明'}</p>
        </CardHeader>
      </Card>
...
/** 渲染议事决策摘要列表。 */
function DecisionList({ matter, decisions }: { matter: MatterDetail; decisions: DecisionSummary[] }) {
  return (
      ...
      {meetings.map((meeting) => (
        <li key={meeting.id} className="rounded-md border p-3">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {meeting.areaName} · {meeting.decisions.length ? `${meeting.decisions.length} 项决策` : '普通议事会议'}
          </p>
        </li>
      ))}
    </ul>
