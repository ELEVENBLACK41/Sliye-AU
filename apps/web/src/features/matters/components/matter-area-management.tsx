...
import {
  ...
  removeAreaMember,
  updateMatterArea,
} from '../services/matters-client.service';
...
/** 私有分区管理组件属性。 */
type MatterAreaManagementProps = {
  matter: MatterDetail;
...
/** 渲染私有分区创建和当前分区维护界面。 */
export function MatterAreaManagement({ matter, members, currentArea, areaMembers }: MatterAreaManagementProps) {
  ...
  function handleCreate(event: FormEvent<HTMLFormElement>): void {
    ...
    void run(async () => {
      await createMatterArea(matter.id, {
        name: name.trim(),
  ...
  function handleAddMember(event: FormEvent<HTMLFormElement>): void {
    ...
    void run(async () => {
      await addAreaMember(matter.id, currentArea.id, { userId: parsedUserId, role: 'MEMBER' });
      setNewMemberId('');
    });
  }
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <Select
              ...
              onValueChange={(status) =>
                void run(() => updateMatterArea(matter.id, currentArea.id, { status: status as DiscussionAreaStatus }))
              }
            >
          ...
          <form className="flex gap-2" onSubmit={handleAddMember}>
            <Select value={newMemberId} onValueChange={setNewMemberId}>
              <SelectTrigger className="flex-1">
                <SelectValue placeholder="选择议事成员" />
              </SelectTrigger>
              <SelectContent>
            ...
            {areaMembers.map((member) => (
              <li key={member.id} className="flex items-center justify-between gap-2 rounded-md border p-3 text-sm">
                ...
                <Button
                  ...
                  variant="destructive"
                  disabled={pending}
                  onClick={() => void run(() => removeAreaMember(matter.id, currentArea.id, member.user.id))}
                >
                  移除
                </Button>
              </li>
