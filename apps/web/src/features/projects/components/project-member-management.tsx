...
import type { ProjectDetail, ProjectMember, ProjectMemberCandidate } from '@workspace/contracts/projects';

import { addProjectMember, removeProjectMember, updateProjectMember } from '../services/projects-client.service';
...
/** 可由普通成员接口维护的项目角色。 */
type MutableProjectMemberRole = 'MANAGER' | 'MEMBER' | 'VIEWER';
...
export function ProjectMemberManagement({
  ...
  const [role, setRole] = useState<MutableProjectMemberRole>('MEMBER');
  ...
  async function run(action: () => Promise<unknown>, successMessage: string): Promise<void> {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : '项目成员操作失败');
    } finally {
      setPending(false);
    }
  }
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>): void {
    ...
    void run(
      async () => {
        await addProjectMember(project.id, { userId: candidate.id, role });
        setSelectedCandidateId(null);
        setSearch('');
      },
      `${candidate.name || candidate.email} 已加入项目`,
    );
  }
    ...
    <div className="space-y-5">
      <form className="grid gap-4" onSubmit={handleSubmit}>
        <div className="grid gap-2">
          <Label htmlFor="project-member-search">选择要加入的用户</Label>
        ...
        <Button disabled={pending || selectedCandidateId === null || project.status !== 'ACTIVE'}>
            ...
            {member.role === 'OWNER' ? (
              ...
              <>
                <MemberRoleSelect
                  value={member.role}
                  onChange={(nextRole) =>
                    void run(() => updateProjectMember(project.id, member.user.id, { role: nextRole }), '成员角色已更新')
                  }
                />
                <Button
                  type="button"
                  ...
                  variant="destructive"
                  disabled={pending}
                  onClick={() => void run(() => removeProjectMember(project.id, member.user.id), '成员已移出项目')}
                >
                  移除
                </Button>
              </>
...
function MemberRoleSelect({
  ...
  value: MutableProjectMemberRole;
  onChange: (role: MutableProjectMemberRole) => void;
}) {
  return (
    <Select value={value} onValueChange={(nextRole) => onChange(nextRole as MutableProjectMemberRole)}>
