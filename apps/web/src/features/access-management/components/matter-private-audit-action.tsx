/**
 * 本文件提供独立的私有议事内容审计入口，要求填写明确目标与访问原因并只读展示结果。
 */
'use client';
...
import { readPrivateMatterContent } from '../services/access-management-client.service';
...
export function MatterPrivateAuditAction() {
  ...
  const [result, setResult] = useState<MatterAuditReadResponse | null>(null);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (![parsedMatterId, parsedAreaId].every((value) => Number.isInteger(value) && value > 0)) return;
    ...
    try {
      setResult(
        await readPrivateMatterContent({
    ...
    <Card className="rounded-md border-amber-500/40 shadow-none">
      ...
      <CardContent className="space-y-4">
        ...
        <form className="grid gap-4 md:grid-cols-3" onSubmit={handleSubmit}>
          <AuditIdField id="audit-matter-id" label="议事 ID" value={matterId} onChange={setMatterId} />
...
/** 展示一次不可编辑的审计读取快照。 */
function AuditResult({ result }: { result: MatterAuditReadResponse }) {
  return (
    <section className="space-y-3 rounded-md border bg-muted/30 p-4" aria-label="审计读取结果">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          {result.matter.title} · {result.area.name}
