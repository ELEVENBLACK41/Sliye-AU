...
/** 渲染由 URL 驱动的 AI Thread 工作区；运行控制和 SSE 留给后续增量。 */
export function AiWorkspace() {
  ...
  return (
    <section
      ...
      aria-label="AI 实验室"
    >
      <AiWorkspaceSidebar data={sidebarData} onThreadSelect={workspace.selectThread} onNewThread={workspace.startNewThread} />
...
/** 渲染 2.6-A 的详情和首屏消息四态；实时画布将在 2.6-B 接入。 */
function AiWorkspaceContent({ workspace }: { workspace: ReturnType<typeof useAiThreadWorkspace> }) {
  ...
  if (workspace.threadLoadState === 'ERROR') {
    return <p role="alert" className="p-6 text-sm text-destructive">{workspace.threadError}</p>;
  }

  if (workspace.messageLoadState === 'ERROR') {
    return <p role="alert" className="p-6 text-sm text-destructive">{workspace.messageError}</p>;
  }
