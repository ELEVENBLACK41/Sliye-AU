...
export function LoginForm({ redirectTo = LOGIN_REDIRECT_PATH }: LoginFormProps) {
  ...
  const [loginValues, setLoginValues] =
    useState<LoginFormValues>(INITIAL_LOGIN_VALUES)
  ...
  const [confirmValues, setConfirmValues] =
    useState<ConfirmEmailFormValues>(INITIAL_CONFIRM_VALUES)
  ...
  const canRegister = useMemo(
    () =>
      ...
      registerValues.password.length >= 8,
    [registerValues.email, registerValues.password],
  )
  const canConfirm = useMemo(
    () =>
      confirmValues.email.trim().length > 0 &&
      /^\d{6}$/.test(confirmValues.code.trim()),
    [confirmValues.code, confirmValues.email],
  )
  const secondsUntilExpiry = emailVerification?.expiresAt
    ? Math.max(
        0,
        Math.ceil((new Date(emailVerification.expiresAt).getTime() - now) / 1000),
      )
    : null
  const cooldownSeconds =
    emailVerification && lastCodeSentAt
      ? Math.max(
          0,
          emailVerification.cooldownSeconds -
            Math.floor((now - lastCodeSentAt) / 1000),
        )
      : 0
        ...
        <form onSubmit={handleConfirm}>
            ...
            {secondsUntilExpiry !== null ? (
              <div className="rounded-md border bg-zinc-50 p-3 text-sm text-muted-foreground">
                ...
                {secondsUntilExpiry === 0 ? (
                  <p className="text-destructive">验证码已过期，请重新发送。</p>
                ) : null}
              </div>
            ) : null}
          ...
          <CardFooter className="flex-col gap-3 pt-6">
            <Button
              type="submit"
              className="w-full"
              disabled={!canConfirm || isSubmitting || secondsUntilExpiry === 0}
            >
              {isSubmitting ? (
                <Loader2 className="size-4 animate-spin" aria-hidden />
              ) : (
